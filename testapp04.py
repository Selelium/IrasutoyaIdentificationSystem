...
def main():
    ...
    if uploaded_file is not None:
        ...
        # 推論で得られたクラス番号(初期値は0)によって出力結果を分岐
        if classNo == 0:
            st.subheader(f"これは{pred0}％の確率で「ネコ」です！")
        elif classNo == 1:
            st.subheader(f"これは{pred1}％の確率で「イヌ」です！")
        else:
            st.subheader(f"これは{pred2}％の確率で「アザラシ」です！")

    # タイトルの表示
    st.title("いらすとや/いらすとん識別システム")
    st.write("画像がいらすとやのものなのかいらすとんのものなのかわからなくなってしまったときに機械の力で客観的に識別してくれるシステム")

    # アップローダの作成
    uploaded_file2 = st.file_uploader("Choose a Image...", type="jpg", key=2)
    ...
    if uploaded_file2 is not None:
        ...
        st.caption(f'判定結果：{classNo2}番')      # 戻り値の確認（デバッグ用）

        # 推論の確率を小数点以下3桁で丸め×100(%に変換)
        pred20 = round(prediction2[0],3) * 100  # 猫の確率(%)
        pred21 = round(prediction2[1],3) * 100  # 犬の確率(%)

        # 推論で得られたクラス番号(初期値は0)によって出力結果を分岐
        if classNo2 == 0:
            st.subheader(f"これは{pred20}％の確率で「いらすとや」です！")
        else:
            st.subheader(f"これは{pred21}％の確率で「いらすとん」です！")
...
# 学科：高度情報工学科
# 学年：4年
# コメント：少ない枚数で学習させてもある程度実用できるレベルの識別力があってすごいと思った
#
#=============================================================================#
